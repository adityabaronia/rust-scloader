...
def convert_bin_to_rust_array(bin_path, array_name="shellcode"):
    shellcode_file = Path("shellcode.txt")

    # Check if the shellcode.txt file exists, and delete it if it does
    if shellcode_file.exists():
        shellcode_file.unlink()
        print(f"[-] Deleted existing shellcode.txt")
...
def create_cargo_project(project_name):
    ...
    if project_path.exists():
        # Delete the existing project folder if it exists
        shutil.rmtree(project_path)
        print(f"[-] Deleted existing project folder: {project_name}")

    # Create a new Cargo project from scratch
